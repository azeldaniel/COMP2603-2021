...
public class Nursery {
    ...
    */    
    public String addPlant(String name, String price, String quantity){
        ...
        if( (plantPrice < 0) || (plantQuantity <0))
         return "Plant "+ name +" not added, invalid data";
    ...
    */
    public String updatePlant(String name, String price, String quantity){
        /*Plant p = new Plant(name);
        ArrayList<Plant> plantsList = (ArrayList)plants;
        int index = plantsList.indexOf(p);
        if(index == -1)
        ...
        //Plant newP = new Plant(name, plantPrice, plantQuantity);
        Plant oldP = plantsList.get(index);
        oldP.setName(name);
        oldP.setPrice(plantPrice);
        oldP.setQuantity(plantQuantity);
        
        return "Updated plant details: "+plantsList.get(index).toString();*/
        
        return "Plant not updated";
    }
    
    public String deletePlant(String name){
        Plant p = new Plant(name);
        boolean deleted = plants.remove(p);
        if(deleted)
    ...
    */
    public String getPlantsByPrice(){
       ...
       if(!plants.isEmpty()){
           ...
           ArrayList<Plant> plantsByPrice = new ArrayList();
           plantsByPrice.addAll(plants);
           Collections.sort(plantsByPrice, sortByPrice);
